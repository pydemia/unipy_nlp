...
def refine_content(df):
    ...
    return dfdef refine_content(df):
...
#%% [markdown]
# #### ci_voice
# Paste 과정에서 줄바꿈이 과도하게 적용된 면이 있음.
ldict['ci_voice']['18년']['content'][:5]#%% [markdown]
# #### blind
ldict['blind']['18년']['content'][:5]#%% [markdown]
# #### retiree_interview
ldict['retiree_interview']['18년']['content'][:5]#%% [markdown]
# #### sharedoffice_survey

#%%
ldict['sharedoffice_survey']['18년']['content'][:5]

#%% [markdown]
# #### usymphony
ldict['usymphony']['Clientes']['reply'][:5]ldict['usymphony']['Clientes'].keys()ddict['usymphony']['Clientes']['content'][0]ldict['usymphony']['Clientes']['content'][:5]#%% [markdown]
# #### reader_survey
ldict['reader_survey']['18년']['content'][:5]#%% [markdown]
# #### culture_survey
ldict['culture_survey']['18년']['content'][:5]#%% [markdown]
# ### Sentencing (Baseline)
...
ptn_a = re.compile(r'[\_\·\à\♬\<\>\(\)\[\]\{\}\*\'\"\-\+\~\|⓪①②③④⑤⑥⑦⑧⑨⑩]+')
ptn_b = re.compile(r'[\^]{2,}')
ptn_c = re.compile(r'[\.{2,}\!\?]+[\s]+|[\s]{3,}')
ptn_d = re.compile(r'([^\s]+음)[\s]+|([^\s]+함)[\s]+|([^\s]+니다)[\s]+|([^\s]+됨)[\s]+')
ptn_dd = re.compile(r'([^\s]+니다)[\s]+')
ptn_e = re.compile(r'[\s]{2,}')
ptn_ee = re.compile(r'(^\d{4} \d{1,2}:\d{2} [ap]{1}mto: .+subject:)')
# pc = re.compile(r'[\.\!\?]+[\s]+')
# 날짜표시용 format 변환 추가 필요
# pa = re.compile(r'\_|\·|\à|\♬|\<|\>|\(|\)|\[|\]|\{|\}|\*|\'|\"|\.|\!|\?|\-|\+')
...
).drop_duplicates(subset='word')udf_token['0'] = udf_token['word']
udf_token['1'] = 0
udf_token['2'] = 0
udf_token['3'] = 0
udf_token['4'] = 'NNG'
...
udf_token['10'] = '*'
udf_token['11'] = '*'
udf_token['11'] = '*'udf_token.columns.str.isnumeric()udf_token_mecab = udf_token.loc[:, udf_token.columns.str.isnumeric()]표층형 (표현형태)	0	0	0	품사 태그	의미 부류	종성 유무	읽기	타입	첫번째 품사	마지막 품사	 	 
서울	          0	0	0	  NNG	지명	T	서울	*	*	*	*	 
불태워졌	     0	0	0	 VV+EM+VX+EP	*	T	불태워졌	inflected	VV	EP	*	불태우/VV/+어/EC/+지/VX/+었/EP/
해수욕장      	0	0	0	 NNG	 	T	해수욕장	Compound	*	*	해수/NNG/+욕/NNG/+장/NNG/*	 # udf_token_mecab.to_csv('/tmp/mecab-ko-dic-2.1.1-20180720/user-dic/nnp.csv', header=False, index=False)
udf_token_mecab.to_csv('~/Downloads/mecab-ko-dic-2.1.1-20180720/user-dic/nnp.csv', header=False, index=False)# !/tmp/mecab-ko-dic-2.1.1-20180720/tools/add-userdic.sh
# !cd /tmp/mecab-ko-dic-2.1.1-20180720;make install
!~/Downloads/mecab-ko-dic-2.1.1-20180720/tools/add-userdic.sh
!cd ~/Downloads/mecab-ko-dic-2.1.1-20180720;sudo make installaa = sorted(sentenced, key=len, reverse=True)
tt = '\n'.join(aa)
#%%
tag_list = [
    '체언 접두사', '명사', '한자', '외국어',
...
# self design
# self-design
bpe_file = 'data/news/newsdata_bpe_200000.piece'
with open(bpe_file, 'r') as file:
    # remove '_io.TextIOWrapper line
    # dd = file.read()
    # bpedata = file.readlines()
    tokenized = [stc.split(' ') for stc in file.readlines()]

# import sentencepiece as spm
# sp = spm.SentencePieceProcessor()
# sp.Load('data/news/news_spm.model')

# %%
tokenized[:2]#%% [markdown]
# ## BOW
...
!unzip mallet-2.0.8.zip# Download File: http://mallet.cs.umass.edu/dist/mallet-2.0.8.zip
mallet_path = 'mallet-2.0.8/bin/mallet' # update this path
ldamallet = gensim.models.wrappers.LdaMallet(
    mallet_path, corpus=bow_corpus, num_topics=5, id2word=cdict,
)

# Show Topics
pprint(ldamallet.show_topics(3, formatted=False))

# Compute Coherence Score
coherence_model_ldamallet = CoherenceModel(
    model=ldamallet, texts=tokenized, dictionary=cdict, coherence='c_v'
)
...
def get_repr_tokenized(repr_sentenced, dictionary=None):
    ...
    return spmed_unspaced, bow_corpus_raw, bow_corpus_idx(tokenized_repr,
 bow_corpus_raw_repr,
